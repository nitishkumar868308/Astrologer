            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                {features.map((feature, index) => (
                    <motion.div
                        ...
                        viewport={{ once: true, amount: 0.3 }}
                        transition={{ duration: 0.5, delay: index * 0.1 }}
                        className="bg-[#fdf8f3] rounded-[50px] shadow-[0_20px_40px_rgba(0,0,0,0.25)] p-8 flex flex-col items-center text-center cursor-pointer hover:scale-105 hover:shadow-[0_25px_50px_rgba(0,0,0,0.35)] transition-all"
                    >
                        <div className="w-20 h-20 mb-6 flex items-center justify-center rounded-full bg-gradient-to-br from-[#d9914b] to-[#b77a3a]">
                            <img src={feature.icon} alt={feature.title} className="w-10 h-10 object-contain" />
                        </div>
